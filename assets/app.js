/**
 * SISTEMA DE LEITURA DE HIDRÔMETROS v2.5.5
 * Correções: mudarLocal, recuperação de select, progresso
 */
const CONFIG = {
...
class HidrometroApp {
    ...
    recuperarRondaImediatamente() {
        ...
        try {
            ...
            // PREENCHE SELECT ANTES DE TUDO
            this.preencherSelectLocais();
            
            // SELECIONA LOCAL E MOSTRA
            const localInicial = this.locais[0] || '';
            console.log('Local inicial:', localInicial);
            
            if (localInicial) {
                const select = document.getElementById('localSelect');
                select.value = localInicial;
                ...
                setTimeout(() => {
                    ...
                    this.atualizarProgresso(); // Força atualização do progresso
    ...
    preencherSelectLocais() {
        const select = document.getElementById('localSelect');
        if (!select) {
            console.error('Select de local não encontrado!');
            return;
        }

        const valorAtual = select.value;
        console.log('Preenchendo select. Locais:', this.locais);

        select.innerHTML = '<option value="">Escolha um local...</option>';
        
        if (!this.locais || this.locais.length === 0) {
            console.warn('Nenhum local disponível');
            return;
        }
        ...
        console.log('Select preenchido com', select.options.length, 'opções');
    }
    ...
    mostrarHidrometrosDoLocal(local) {
        ...
        if (!local) {
            console.warn('Local vazio');
            return;
        }

        const hidrometrosLocal = this.hidrometros.filter(h => h.local === local);
        console.log(`Mostrando ${hidrometrosLocal.length} hidrômetros do local ${local}`);
    ...
    // ALIAS para compatibilidade com HTML existente
    mudarLocal(select) {
    ...
    changeLocal(select) {
        ...
        console.log('Mudando para local:', local);
        if (local) {
    ...
    setupEventListeners() {
        ...
        if (localSelect) {
            // Remove onchange inline se existir
