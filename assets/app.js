 ...
 * Correção: Recuperação automática ao recarregar (F5 normal) sem zerar dados salvos
 */
const CONFIG = {
        ...
        if (salvo) {
            try {
                ...
                if (this.usuario.nivel === 'admin') {
                    ...
                    // Verifica se tem ronda pendente ANTES de mostrar startScreen
                    const rondaSalva = localStorage.getItem(CONFIG.STORAGE_KEYS.RONDA_ATUAL);
                    if (rondaSalva) {
                        console.log('Ronda pendente encontrada no login. Recuperando diretamente.');
                        this.resumeRondaIfExists();  // Vai direto para leituraScreen
                    } else {
    ...
    resumeRondaIfExists() {
        ...
        try {
            ...
            if (this.hidrometros.length > 0) {
                console.log(`✅ Recuperando ronda pendente diretamente (${this.hidrometros.length} hidrômetros)`);
