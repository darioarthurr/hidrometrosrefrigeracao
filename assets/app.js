/**
 * SISTEMA DE LEITURA DE HIDRÔMETROS v2.0
...
class HidrometroApp {
    ...
    async loadDashboard() {
        ...
        try {
            ...
            if (cache) {
                ...
                if (new Date() - new Date(dadosCache.timestamp) < 5 * 60 * 1000) { // 5 min
    ...
    exportarExcel() {
        ...
        // Pega todos os dados, não só a página atual
        // Simplificação: exporta o que está visível
        Array.from(tabela.rows).forEach(row => {
            const cells = Array.from(row.cells).slice(0, -1); // Remove coluna ações
