/**
 * SISTEMA DE LEITURA DE HIDRÔMETROS v2.5.4
 * JavaScript Completo - Offline First, Dashboard Admin, PWA
 * Correção: Recuperação automática ao recarregar (F5) funcionando 100%
 */
const CONFIG = {
...
class HidrometroApp {
    constructor() {
        ...
        this._inicializando = false; // Flag para evitar dupla inicialização
        ...
        if (salvo) {
            try {
                ...
                if (this.usuario.nivel === 'admin') {
                    ...
                    // VERIFICA PRIMEIRO SE TEM RONDA SALVA
                    const temRonda = this.verificarRondaSalva();
                    
                    if (temRonda) {
                        // Se tem ronda, recupera ela diretamente (não mostra startScreen)
                        console.log('📦 Ronda em andamento detectada. Recuperando...');
                        this.recuperarRondaImediatamente();
                    } else {
                        // Se não tem ronda, mostra tela inicial normal
    ...
    // NOVO: Verifica se existe ronda válida salva
    verificarRondaSalva() {
    ...
    // NOVO: Recupera ronda diretamente sem passar pela startScreen
    recuperarRondaImediatamente() {
        if (this._inicializando) return; // Evita dupla execução
        ...
        try {
            ...
            console.log(`✅ Ronda ${this.rondaAtual} recuperada: ${lidos}/${this.hidrometros.length} lidos`);

            // Mostra tela de leitura DIRETAMENTE
            this.showScreen('leituraScreen');
            document.getElementById('bottomBar').style.display = 'block';

            // Preenche o select de locais
            this.preencherSelectLocais();
            
            // Seleciona o primeiro local ou o último que estava sendo editado
            const localInicial = this.locais[0] || '';
            if (localInicial) {
                document.getElementById('localSelect').value = localInicial;
                this.mostrarHidrometrosDoLocal(localInicial);
                
                // Aguarda renderização e restaura dados
                setTimeout(() => {
                    this.restaurarTodosOsDados();
                    this.showToast(`Ronda recuperada (${lidos} de ${this.hidrometros.length})`, 'success');
                }, 100);
            }
            ...
            localStorage.removeItem(CONFIG.STORAGE_KEYS.RONDA_ATUAL);
            // Se falhar, volta para startScreen
            this.showScreen('startScreen');
            document.getElementById('nomeTecnico').textContent = this.usuario.nome;
        } finally {
            this._inicializando = false;
    ...
    async login(e) {
        ...
        try {
            ...
            if (data.nivel === 'admin') {
                ...
                // Após login, verifica se tem ronda pendente
                const temRonda = this.verificarRondaSalva();
                if (temRonda) {
                    const continuar = confirm('Você tem uma ronda em andamento. Deseja continuar de onde parou?');
                    if (continuar) {
                        this.recuperarRondaImediatamente();
                    } else {
                        // Limpa ronda antiga e vai para start
                        localStorage.removeItem(CONFIG.STORAGE_KEYS.RONDA_ATUAL);
                        this.showScreen('startScreen');
    ...
    // Mantido para compatibilidade, mas não é mais usado no checkAuth
    resumeRondaIfExists() {
        this.recuperarRondaImediatamente();
    }
    ...
    salvarRondaLocal() {
        ...
        try {
            ...
            this._salvamentoPendente = false;
            const lidos = this.hidrometros.filter(h => h.leituraAtual > 0).length;
            console.log(`💾 Ronda salva: ${lidos}/${this.hidrometros.length} lidos`);
        } catch (e) {
    ...
    restaurarTodosOsDados() {
        ...
        console.log(`✅ Restaurado: ${restaurados} leituras, ${comFoto} fotos, ${comJustificativa} justificativas`);
    }
